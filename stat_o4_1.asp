...
db.Open "ATOC1"   
Name_Zad="ATOC_o4_1"   
pSlv="n1_213"   'национальность---изменяя словарь получишь другую статистику
POLE="r13"       'изменяя поле получишь другую статистику
...
sub SPISOK_PRINT
   Response.Write "<TABLE border=><TR><TD>национальность<TD>всего"
   S=1
   KOL=0
   Do while S<999
      ...
      ZNA=MAC(S)
      if ZNA>0 then
         ZNA="<A href='STAT_o4_1.asp?ORGAN=" & S & "'>&nbsp;<FONT size=+1>" & ZNA & "</FONT>&nbsp;</A>"
         Response.Write "<TR><TD>" & Slv_Get(pSlv,S) & "<TD align=right>" & ZNA
         KOL=KOL+1
         if (KOL=19)or(KOL=38)or(KOL=57)or(KOL=76)or(KOL=95)or(KOL=114)or(KOL=133)or(KOL=152)or(KOL=171)or(KOL=190) then Response.Write "</TABLE><TABLE border=1 align=left><TR><TD>национальность<TD>всего"
      end if
      S=S+1
...
sub ORGAN_SHET(ORGAN)
   ...
   Do while NOT Rs.EOF 
      ...
      KOL_ZAP = KOL_ZAP + 1 
      Response.Write "<TR><TD align=right><A href='vid-o4.asp?N=" & N & "'>"& KOL_ZAP&"&nbsp;</A>"
