...
<%
' ПОЛЯ В О2 НЕТУ r04=rs.Fields("r04").value
sub VID_AI
...
'        Response.Write "<TR><TD align=right >""<div id='blink7'>" Архивный номер <TD> выделено <TD> прек по статье <TD> дата <TD> статья <TD> в отношении"</DIV>"
         Response.Write "<TR bgcolor=#FFD700 ><TD align=right > Архивный номер </TD><TD> выделено </TD><TD> прек по статье </TD><TD> дата </TD><TD> статья </TD><TD> в отношении</TD </DIV></TR>"
'         Response.Write "<DIV align=center Color=Blue> <FONT color=firebrick> |Архивный номер|выделено|прек по статье|дата|статья|в отношении|"     ' LightCyan
         Response.Write "<TR bgcolor=#F4A460 <div id='blink-2' ><TD  align=right><A href='Vid-AI.asp?N=" & N_AI & "' target='_blank'>" & PR & " &nbsp;</A></DIV>"
...
if NOT Rs.EOF then
   ...
   Set rs2 = db.Execute("Select * From ATOC_o2_4 where AT_1=" & N & ";")
...
' 16_04_2021 все что ниже предназначено для VID_AI
      ...
      rG=rs2.Fields("rG").value
      rN=rs2.Fields("rN").value
'      r04=rs2.Fields("r04").value  ЭТОГО ПОЛЯ В О2 НЕТУ ЖЕ
      call VID_AI
      Response.Write "<Table border=1 cellspacing=0 bgColor=Seashell>"  
